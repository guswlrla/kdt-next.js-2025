...
import restaurantData from "@/data/r.json";

import fs from 'fs/promises';
import path from 'path';

const getFilePath = () => path.join(process.cwd(), 'src', 'data', 'r.json');
...
async function getData(): Promise<restaurantData[]> {
  const jsonData = await fs.readFile(getFilePath(), 'utf-8');
  return JSON.parse(jsonData);
}

async function writeData(data: restaurantData[]) {
...
export async function GET(request: NextRequest) {
  ...
  if (id) {
    const idData = restaurantData.find(item => item.UC_SEQ === Number(id));
    if (idData) return NextResponse.json(idData);
    return NextResponse.json({ error: `${id}(을)를 찾을 수 없습니다!` }, { status: 404 });
  }
  else return NextResponse.json(restaurantData);
}

export async function POST(request: NextRequest) {
  try {
    const newData = await request.json();
    let existingData: restaurantData[] = []; // 기존 자료 읽기
